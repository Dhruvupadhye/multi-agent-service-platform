...
| 📧 **Smart Email Agent** | Fetches unread emails via the Gmail API, filters out spam using a custom-trained ML classifier, and summarizes the rest. |
...
| 📚 **Document RAG Agent** | Upload PDFs to build a local knowledge base (ChromaDB) and ask questions about your documents. Falls back to web search if the answer isn't found locally. |
| ✍️ **Email Composer Agent** | Drafts professional emails based on your voice instructions, pulling from your document knowledge base when needed. |
...
1. Download your OAuth 2.0 Client IDs from Google Cloud Console.
2. Save the file as `credentials.json` in the root directory.
3. On first run, the app will open a browser window to authenticate and generate `token.json` and `calendar_token.json`.

### 6. Train the Spam Classifier
Before running the platform, generate the `.pkl` model file used by the Email Agent:
